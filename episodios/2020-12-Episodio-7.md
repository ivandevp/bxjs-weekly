---
title: "Episodio 7 - Semana 12, 2020"
description: "
<p>Esta semana entérate de la unión de NPM a Github, aprende cómo hacer una API Rest con Deno y Postgres, prueba la aplicación móvil de Github, conoce los releases de Babel, React Scripts y Prettier, &nbsp;sigue el stream de Node.js En Vivo con <a href='https://twitter.com/julian_duque'>Julián Duque</a>.</p>"
descriptionText: "Esta semana entérate de la unión de NPM a Github, aprende cómo hacer una API Rest con Deno y Postgres, prueba la aplicación móvil de Github, conoce los releases de Babel, React Scripts y Prettier,  sigue el stream de Node.js En Vivo con Julián Duque."
date: "2020-03-22"
image: "https://s3-us-west-2.amazonaws.com/anchor-generated-image-bank/production/podcast_uploaded_episode400/3216485/3216485-1584909611212-4301a13bdf3c3.jpg"
audio: "
<iframe src='https://anchor.fm/unexpected-news/embed/episodes/Episodio-7---2020-Semana-12-ebq1b2' height='102px' width='400px' frameborder='0' scrolling='no'></iframe>"
---

# Episodio 7 - Semana 12, 2020

## Guías de inicio

- [AngularJS Security Fundamentals](https://snyk.io/blog/angularjs-security/)
- [Everything About Auto in CSS](https://ishadeed.com/article/auto-css/)
- [Introduction to AntV](https://observablehq.com/@jiazhewang/introduction-to-antv)
- [React Suspense in Practice](https://css-tricks.com/react-suspense-in-practice/)
- [Javascript Proxies : Real World Use Cases](https://www.arbazsiddiqui.me/javascript-proxies-real-world-use-cases/)

## Artículos y noticias

- [npm is joining GitHub](https://github.blog/2020-03-16-npm-is-joining-github/)
- [Why we chose Typescript for the Hasura Console](https://hasura.io/blog/why-we-chose-typescript-for-hasura-console/)
- [GitHub Mobile is officially leaving beta](https://twitter.com/github/status/1239968032325156864)
- [Electron is temporarily pausing major releases, in conjunction with Chromium’s announcement](https://twitter.com/electronjs/status/1240768283336298496)

## Tutoriales

- [Creating your first REST API with Deno and Postgres](https://blog.logrocket.com/creating-your-first-rest-api-with-deno-and-postgres/)
- [Implementing Infinite Scroll And Image Lazy Loading In React](https://www.smashingmagazine.com/2020/03/infinite-scroll-lazy-image-loading-react/)
- [Using watchers for form input validation in Vue.js](https://blog.logrocket.com/form-input-validation-in-vue-js-using-watchers/)
- [React Native Build & Validate Forms with Formik & Yup](https://www.positronx.io/react-native-build-validate-forms-with-formik-yup/)

## Releases

- [Babel 7.9.0](https://babeljs.io/blog/2020/03/16/7.9.0)
- [React Scripts 3.4.1](https://github.com/facebook/create-react-app/releases/tag/v3.4.1)
- [Prettier 2.0](https://prettier.io/blog/2020/03/21/2.0.0.html)

## Libs

- [rfdc](https://github.com/davidmarkclements/rfdc)
- [heapify](https://github.com/luciopaiva/heapify)
- [useWorker](https://github.com/alewin/useWorker)
- [Tailwind Starter Kit](https://www.creative-tim.com/learning-lab/tailwind-starter-kit#/presentation)
- [gqless](https://gqless.netlify.com/)

## Streams

- [Node.js En Vivo - Base de Datos PostgreSQL](https://www.twitch.tv/videos/572155558)

## Curiosidades

- [Flexbox30](https://www.samanthaming.com/flexbox30/)
- [SVG Favicons](https://twitter.com/LeaVerou/status/1241619866475474946?s=20)
- [browser-2020](https://github.com/luruke/browser-2020)
- [Animated hamburger icons](https://hamburger-react.netlify.com/)
