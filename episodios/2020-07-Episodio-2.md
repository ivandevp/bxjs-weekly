---
title: "Episodio 2 - Semana 7, 2020"
description: "
<p>Esta semana el lazy loading de imágenes se aceptó como parte del estándar de HTML5, la librería request de Node.js fue oficialmente deprecada, Github CLI sale en versión beta y ahora puedes crear PRs desde la terminal, Ionic lanzó su versión 5.</p>"
descriptionText: "Esta semana el lazy loading de imágenes se aceptó como parte del estándar de HTML5, la librería request de Node.js fue oficialmente deprecada, Github CLI sale en versión beta y ahora puedes crear PRs desde la terminal, Ionic lanzó su versión 5."
date: "2020-02-17"
image: "https://s3-us-west-2.amazonaws.com/anchor-generated-image-bank/production/podcast_uploaded_episode400/3216485/3216485-1581927178502-ba8c2aaead909.jpg"
audio: "
<iframe src='https://anchor.fm/unexpected-news/embed/episodes/Episodio-2---2020-Semana-7-easm7p' height='102px' width='400px' frameborder='0' scrolling='no'></iframe>
"
---

# Episodio 2 - Semana 7, 2020

## Guías de inicio

- [Aborting a fetch request](https://meetguns.com/blog/aborting-a-fetch-request/)
- [Understanding the Node.js event loop phases and how it executes the JavaScript code](https://dev.to/lunaticmonk/understanding-the-node-js-event-loop-phases-and-how-it-executes-the-javascript-code-1j9)
- [What is really so special about JavaScript Closures?](https://levelup.gitconnected.com/what-is-really-so-special-about-javascript-closures-5701f8e4229f)
- [6 things I wish I knew about state management when I started writing React apps](https://medium.com/@veeralpatel/things-ive-learned-about-state-management-for-react-apps-174b8bde87fb)
- [CSS Variables](https://dev.to/samanthaming/css-variables-8di)
- [A Collection of TypeScript Guides and Cheatsheets](https://dev.to/jamenamcinteer/a-collection-of-typescript-guides-and-cheatsheets-288n)
- [Nullish coalescing with React and TypeScript](https://www.carlrippon.com/nullish-coalescing-with-react-and-typescript/)

## Artículos y noticias

- [Array.reduce callback is called with the wrong accumulator](https://bugs.chromium.org/p/chromium/issues/detail?id=1049982)
- [Building the web we want](https://css-tricks.com/building-the-web-we-want/)
- [Specification for native image lazy-loading](https://twitter.com/addyosmani/status/1227619409625174016?s=12)
- [`request` library now depecrated](https://github.com/request/request#deprecated)
- [Supercharge your command line experience: GitHub CLI is now in beta](https://github.blog/2020-02-12-supercharge-your-command-line-experience-github-cli-is-now-in-beta/)
- [The Babel team contributes export type { T } for TypeScript](https://www.youtube.com/watch?v=qYJxFtbgzKM)
- [Node.js Async Best Practices & Avoiding the Callback Hell](https://blog.risingstack.com/node-js-async-best-practices-avoiding-callback-hell-node-js-at-scale/)
- [Under Discussion: The Maintenance of Large Open-Source Projects](https://www.welcometothejungle.com/en/articles/btc-discussion-open-source-maintenance)
- [An interview with Eduardo from the Vue.js core team](https://www.youtube.com/watch?v=LKioMRuiAaE)
- [Replacing Styled Components with a 1KB alternative Goober](https://blog.agney.dev/replacing-sc-with-goober/)
- [Applying CSS :focus-within](https://www.giftegwuenu.com/applying-css-focus-within/)
- [JavaScript Interview Questions](https://dev.to/a0viedo/javascript-interview-questions-17je)

## Tutoriales

- [7 JavaScript Utility Functions to Improve Your Efficiency](https://medium.com/javascript-in-plain-english/7-javascript-utility-functions-to-improve-your-efficiency-79d27132f186)
- [Build a very basic SPA JavaScript router](https://dev.to/pixari/build-a-very-basic-spa-javascript-router-2k4p)

## Releases

- [Firefox 73](https://www.mozilla.org/en-US/firefox/73.0/releasenotes/)
- [Ionic 5](https://ionicframework.com/blog/announcing-ionic-5/)

## Libs

- [use-places-autocomplete](https://github.com/wellyshen/use-places-autocomplete)
- [type-route](https://github.com/type-route/type-route)

## Curiosidades

- [explainshell](https://explainshell.com/)
- [gitexlorer](https://gitexplorer.com/)
- [GitHub Enterprise is now free through Microsoft for Startups](https://github.blog/2020-02-13-github-enterprise-is-now-free-through-microsoft-for-startups/)
- [CSS `revert` value](https://twitter.com/mgechev/status/1227495088806187008?s=20)
- [JavaScript folks sharing their job's compensation](https://twitter.com/youyuxi/status/1228904570480398337?s=20)
- [Redefine let/const variables](https://twitter.com/JoshWComeau/status/1227988402584182784?s=20)
- [plink-plonk.js](https://gist.github.com/tomhicks/6cb5e827723c4eaef638bf9f7686d2d8)
